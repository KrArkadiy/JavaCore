...
//Простой пример программы, позволяющий экспериментировать с эффектом от изменения порогового значения
// и уровня параллелизма выполнения задач в классе ForkJoinTask
...
class Transform extends RecursiveAction {
    ...
    //определить часть обрабываемых даннх
    int start, end;
    ...
    protected void compute() {
        ...
        if ((end - start) < seqThreshold) {
            //В следующем фрганменте кода элементу по четному индексу присваивается квадратный корень его первоначального
            //значения, а элементу по нечетному индексу - кубический корень его первоначального значения.
            //Этот код предназначен только для потребления времени ЦП, чтобы сделать нагляднее эффект от параллельного
            //выполнения
            ...
            //В противонм случае продолжить разделение данных на меньшие части
